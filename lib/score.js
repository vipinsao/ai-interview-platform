...
/**
 * Where each rating bucket's number came from.
 *
 * `aggregateScores` falls back to the overall mean when no question of that
 * type was asked, so "Communication: 7.5" can be reported for an interview
 * containing no communication questions at all. That is a reasonable default —
 * a blank is less useful than an estimate — but presenting an estimate as a
 * measurement is not, so the UI marks it.
 *
 * @returns {Record<string, "measured"|"inferred"|"none">}
 */
export function ratingProvenance(answers) {
  const scored = (Array.isArray(answers) ? answers : []).filter(
    (a) => a && typeof a.score === "number" && Number.isFinite(a.score)
  );
  ...
  const provenance = {};
  for (const key of RATING_KEYS) {
    const direct = scored.some((a) => ratingKeyForType(a.type) === key);
    if (direct) provenance[key] = "measured";
    else if (scored.length > 0) provenance[key] = "inferred";
    else provenance[key] = "none";
  }
  return provenance;
}
